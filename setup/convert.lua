...
local function Upgrade()
	...
	local trunk = MySQL.query.await('SELECT owner, name, data FROM bpt_inventory WHERE name LIKE ?', {'trunk-%'})
	local glovebox = MySQL.query.await('SELECT owner, name, data FROM bpt_inventory WHERE name LIKE ?', {'glovebox-%'})
	...
	if trunk and glovebox then
		...
		MySQL.prepare.await('DELETE FROM bpt_inventory WHERE name LIKE ? OR name LIKE ?', {'trunk-%', 'glovebox-%'})
...
local function Convert_Old_ESX_Property()
	...
	MySQL.prepare.await('INSERT INTO bpt_inventory (owner,name,data) VALUES (?,?,?)', parameters)
