...
lib.callback.register('bpt_inventory:openCraftingBench', function(source, id, index)
	...
	if bench then
		...
		if left.open and left.open ~= source then
			local inv = Inventory(left.open) --[[@as BptInventory]]
...
lib.callback.register('bpt_inventory:craftItem', function(source, id, index, recipeId, toSlot)
	local left, bench = Inventory(source), CraftingBenches[id]
	...
	if bench then
		...
		if recipe then
			...
			local success = lib.callback.await('bpt_inventory:startCrafting', source, id, recipeId)
