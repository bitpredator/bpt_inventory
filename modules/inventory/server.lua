...
---@type table<any, BptInventory>
local Inventories = {}

---@class BptInventory
local BptInventory = {}
BptInventory.__index = BptInventory

---Open a player's inventory, optionally with a secondary inventory.
---@param inv? inventory
function BptInventory:openInventory(inv)
	...
	TriggerEvent('bpt_inventory:openedInventory', self.id, inv.id)
end

---Close a player's inventory.
---@param noEvent? boolean
function BptInventory:closeInventory(noEvent)
	...
	if not noEvent then
		TriggerClientEvent('bpt_inventory:closeInventory', self.id, true)
	end

	TriggerEvent('bpt_inventory:closedInventory', self.id, inv.id)
end
...
function BptInventory:syncSlotsWithPlayer(slots, weight)
	TriggerClientEvent('bpt_inventory:updateSlots', self.id, slots, weight)
end

---Sync an inventory's state with all player's accessing it.
---@param slots updateSlot[]
---@param syncOwner? boolean
function BptInventory:syncSlotsWithClients(slots, syncOwner)
	for playerId in pairs(self.openedBy) do
		if self.id ~= playerId then
            local target = Inventories[playerId]

            if target then
			    TriggerClientEvent('bpt_inventory:updateSlots', playerId, slots, target.weight)
            end
		end
	end

	if syncOwner and self.player then
		TriggerClientEvent('bpt_inventory:updateSlots', self.id, slots, self.weight)
	end
end
...
local function loadInventoryData(data, player)
	...
	if data.type == 'trunk' or data.type == 'glovebox' then
		...
		if not inventory then
			...
			local model, class = lib.callback.await('bpt_inventory:getVehicleData', source, data.netid)
...
---@cast Inventory +fun(inv: inventory, player?: inventory): BptInventory|false|nil
...
function Inventory.CloseAll(inv, ignoreId)
	if not inv then
		...
		return TriggerClientEvent('bpt_inventory:closeInventory', -1, true)
	end

	inv = Inventory(inv) --[[@as BptInventory?]]
...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		TriggerClientEvent('bpt_inventory:itemNotify', inv.id, { currentSlot, 'ui_removed', currentSlot.count })
		currentSlot = nil
	else
		...
		TriggerClientEvent('bpt_inventory:itemNotify', inv.id, { currentSlot, count < 0 and 'ui_removed' or 'ui_added', math.abs(count) })
	end
...
CreateThread(function()
    ...
    TriggerEvent('bpt_inventory:loadInventory', Inventory)
end)
...
--- To create a stash, please use `exports.bpt_inventory:RegisterStash` instead.
function Inventory.Create(id, label, invType, slots, weight, maxWeight, owner, items, groups)
	...
	Inventories[self.id] = setmetatable(self, BptInventory)
...
function Inventory.Remove(inv)
	...
	if inv then
		if inv.type == 'drop' then
			TriggerClientEvent('bpt_inventory:removeDrop', -1, inv.id)
...
function Inventory.SetMaxWeight(inv, maxWeight)
    ...
    if inv.player then
        TriggerClientEvent('bpt_inventory:refreshMaxWeight', inv.id, {inventoryId = inv.id, maxWeight = inv.maxWeight})
    end

    for playerId in pairs(inv.openedBy) do
        if playerId ~= inv.id then
            TriggerClientEvent('bpt_inventory:refreshMaxWeight', playerId, {inventoryId = inv.id, maxWeight = inv.maxWeight})
        end
	end
end
		...
		elseif itemSlots and totalCount > 0 then
			for k, v in pairs(itemSlots) do
				if removed < total then
					if v == count then
						TriggerClientEvent('bpt_inventory:itemNotify', inv.id, { inv.items[k], 'ui_removed', v })
...
---@param metadata { [string]: any }
---@param slot number
RegisterServerEvent('bpt_inventory:removeItem', function(name, count, metadata, slot)
...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	TriggerClientEvent('bpt_inventory:createDrop', -1, dropId, Inventory.Drops[dropId])

    return dropId
end

AddEventHandler('bpt_inventory:customDrop', CustomDrop)
...
---@param source number
---@param playerInventory BptInventory
...
local function dropItem(source, playerInventory, fromData, data)
	...
	TriggerClientEvent('bpt_inventory:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
...
---@param source number
---@param data SwapSlotData
lib.callback.register('bpt_inventory:swapItems', function(source, data)
		...
		if fromData and (not fromData.metadata.container or fromData.metadata.container and toInventory.type ~= 'container') then
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						if fromOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', fromData.count })
							TriggerClientEvent('bpt_inventory:itemNotify', fromInventory.id, { toData, 'ui_added', toData.count })
						elseif toOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', toInventory.id, { fromData, 'ui_added', fromData.count })
							TriggerClientEvent('bpt_inventory:itemNotify', toInventory.id, { toData, 'ui_removed', toData.count })
						end
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
						elseif toOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', toInventory.id, { toData, 'ui_added', data.count })
						end
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
						elseif toOtherPlayer then
							TriggerClientEvent('bpt_inventory:itemNotify', toInventory.id, { fromData, 'ui_added', data.count })
						end
			...
			if toInventory.weapon == data.toSlot then
				if not sameInventory then
					...
					TriggerClientEvent('bpt_inventory:disarm', toInventory.id)
				else
			...
			if fromInventory.weapon == data.fromSlot then
				if not sameInventory then
					...
					TriggerClientEvent('bpt_inventory:disarm', fromInventory.id)
...
function Inventory.Confiscate(source)
	...
	if inv?.player then
		...
		TriggerClientEvent('bpt_inventory:inventoryConfiscated', inv.id)
...
function Inventory.Return(source)
	...
	local items = MySQL.scalar.await('SELECT data FROM bpt_inventory WHERE name = ?', { inv.owner })

    if not items then return end

	MySQL.update.await('DELETE FROM bpt_inventory WHERE name = ?', { inv.owner })
    ...
    TriggerClientEvent('bpt_inventory:inventoryReturned', source, { inventory, totalWeight })
...
function Inventory.GetSlotForItem(inv, itemName, metadata)
	local inventory = Inventory(inv)
	local item = Items(itemName) --[[@as BptServerItem?]]
...
RegisterServerEvent('bpt_inventory:closeInventory', function()
...
RegisterServerEvent('bpt_inventory:giveItem', function(slot, target, count)
...
lib.callback.register('bpt_inventory:updateWeapon', updateWeapon)

RegisterNetEvent('bpt_inventory:updateWeapon', function(action, value, slot, specialAmmo)
	updateWeapon(source, action, value, slot, specialAmmo)
end)

lib.callback.register('bpt_inventory:removeAmmoFromWeapon', function(source, slot)
