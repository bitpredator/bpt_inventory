...
function Inventory.GetSlotWithItem(itemName, metadata, strict)
	...
	local item = Items(itemName) --[[@as BptClientItem?]]
                        ...
                        stash.zoneId = Utils.CreateBoxZone(stash.target, {
                            {
                                ...
                                groups = stash.groups,
                                onSelect = function()
                                    exports.bpt_inventory:openInventory('stash', stash.name)
                                end,
...
RegisterNetEvent('bpt_inventory:refreshMaxWeight', function(data)
