...
local Query = {
    SELECT_STASH = 'SELECT data FROM bpt_inventory WHERE owner = ? AND name = ?',
    UPDATE_STASH = 'UPDATE bpt_inventory SET data = ? WHERE owner = ? AND name = ?',
    UPSERT_STASH = 'INSERT INTO bpt_inventory (data, owner, name) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data)',
    INSERT_STASH = 'INSERT INTO bpt_inventory (owner, name) VALUES (?, ?)',
...
Citizen.CreateThreadNow(function()
    ...
    local success, result = pcall(MySQL.scalar.await, 'SELECT 1 FROM bpt_inventory')

    if not success then
        MySQL.query([[CREATE TABLE `bpt_inventory` (
	...
	if clearStashes ~= '' then
		pcall(MySQL.query.await, ('DELETE FROM bpt_inventory WHERE lastupdated < (NOW() - INTERVAL %s)'):format(clearStashes))
	end
end)
