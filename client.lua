...
RegisterNetEvent('bpt_inventory:disarm', function(noAnim)
	currentWeapon = Weapon.Disarm(currentWeapon, noAnim)
end)

RegisterNetEvent('bpt_inventory:clearWeapons', function()
...
function client.openInventory(inv, data)
	...
	if canOpenInventory() then
		...
		if inv == 'shop' and invOpen == false then
			...
			left, right = lib.callback.await('bpt_inventory:openShop', 200, data)
		elseif inv == 'crafting' then
			if cache.vehicle then
				return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
			end

			left = lib.callback.await('bpt_inventory:openCraftingBench', 200, data.id, data.index)
		...
		elseif invOpen ~= nil then
			...
			left, right = lib.callback.await('bpt_inventory:openInventory', false, inv, data)
		end
...
RegisterNetEvent('bpt_inventory:openInventory', client.openInventory)
exports('openInventory', client.openInventory)

RegisterNetEvent('bpt_inventory:forceOpenInventory', function(left, right)
...
lib.callback.register('bpt_inventory:usingItem', function(data)
	local item = Items[data.name]

	if item and usingItem then
		if not item.client then return true end
		---@cast item +BptClientProps
		item = item.client
...
local function useItem(data, cb)
    ...
    usingItem = true
    result = lib.callback.await('bpt_inventory:useItem', 200, data.name, data.slot, slotData.metadata)
...
AddEventHandler('bpt_inventory:item', useItem)
		...
		elseif currentWeapon then
			if data.ammo then
				...
				useItem(data, function(resp)
					...
					lib.callback.await('bpt_inventory:updateWeapon', false, 'load', newAmmo, false, currentWeapon.metadata.specialAmmo)
				end)
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							...
							useItem(data, function(data)
								if data then
									local success = lib.callback.await('bpt_inventory:updateWeapon', false, 'component', tostring(data.slot), currentWeapon.slot)
...
function client.closeInventory(server)
	...
	if invOpen then
		...
		if not server and currentInventory then
			TriggerServerEvent('bpt_inventory:closeInventory')
		end
...
RegisterNetEvent('bpt_inventory:closeInventory', client.closeInventory)
...
local function updateInventory(data, weight)
		...
		if not v.inventory or v.inventory == cache.serverId then
			...
			if currentWeapon?.slot == item?.slot then
                if item.metadata then
				    ...
				    TriggerEvent('bpt_inventory:currentWeapon', currentWeapon)
                else
	...
	for itemName, count in pairs(itemCount) do
        ...
        if item then
            ...
            TriggerEvent('bpt_inventory:itemCount', item.name, item.count)
	...
	TriggerEvent('bpt_inventory:updateInventory', changes)
end

RegisterNetEvent('bpt_inventory:updateSlots', function(items, weights)
	if source ~= '' and next(items) then updateInventory(items, weights) end
end)

RegisterNetEvent('bpt_inventory:inventoryReturned', function(data)
	if source == '' then return end
...
RegisterNetEvent('bpt_inventory:inventoryConfiscated', function(message)
	if source == '' then return end
...
RegisterNetEvent('bpt_inventory:createDrop', function(dropId, data, owner, slot)
...
RegisterNetEvent('bpt_inventory:removeDrop', function(dropId)
	if client.drops then
...
RegisterNetEvent('bpt_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	if source == '' then return end
	...
	for _, v in pairs(Items --[[@as table<string, BptClientItem>]]) do
	...
	local function nearbyLicense(point)
		...
		if point.isClosest and point.currentDistance < 1.2 then
			...
			if IsControlJustReleased(0, 38) then
				lib.callback('bpt_inventory:buyLicense', 1000, function(success, message)
					if success ~= nil then
						lib.notify({
							id = message,
	...
	TriggerEvent('bpt_inventory:updateInventory', PlayerData.inventory)
		...
		if invOpen then
			...
			if currentWeapon and currentWeapon.timer then
				...
				if not invBusy and currentWeapon.timer ~= 0 and currentWeapon.timer < GetGameTimer() then
					...
					if weaponAmmo then
						TriggerServerEvent('bpt_inventory:updateWeapon', 'ammo', weaponAmmo)
					...
					elseif currentWeapon.metadata.durability then
						TriggerServerEvent('bpt_inventory:updateWeapon', 'melee', currentWeapon.melee)
				...
				elseif currentWeapon.throwable then
					if not invBusy and IsControlPressed(0, 24) then
						...
						CreateThread(function()
							...
							TriggerServerEvent('bpt_inventory:updateWeapon', 'throw', nil, weapon.slot)

							plyState.invBusy = false
							currentWeapon = nil

							RemoveWeaponFromPed(playerPed, weapon.hash)
							TriggerEvent('bpt_inventory:currentWeapon')
						end)
					end
...
RegisterNetEvent('bpt_inventory:viewInventory', function(data)
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if not currentWeapon then
		return TriggerServerEvent('bpt_inventory:updateWeapon', 'component', data)
	end
		...
		if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
			for k, v in pairs(itemSlot.metadata.components) do
				if v == data.component then
					local success = lib.callback.await('bpt_inventory:updateWeapon', false, 'component', k)
...
RegisterNUICallback('removeAmmo', function(slot, cb)
	...
	local success = lib.callback.await('bpt_inventory:removeAmmoFromWeapon', false, slot)
...
local function giveItemToTarget(serverId, slotId, count)
    ...
    TriggerServerEvent('bpt_inventory:giveItem', slotId, serverId, count or 0)
end
...
RegisterNUICallback('giveItem', function(data, cb)
	...
	if client.giveplayerlist then
		...
		lib.registerMenu({
			id = 'bpt_inventory:givePlayerList',
			title = 'Give item',
			options = giveList,
		}, function(selected)
		...
		return lib.showMenu('bpt_inventory:givePlayerList')
	end
...
lib.callback.register('bpt_inventory:startCrafting', function(id, recipe)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	local success, response, weaponSlot = lib.callback.await('bpt_inventory:swapItems', false, data)
    swapActive = false
...
RegisterNUICallback('buyItem', function(data, cb)
	...
	local response, data, message = lib.callback.await('bpt_inventory:buyItem', 100, data)
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		local success, response = lib.callback.await('bpt_inventory:craftItem', 200, id, index, data.fromSlot, data.toSlot)
...
lib.callback.register('bpt_inventory:getVehicleData', function(netid)
