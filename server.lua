...
if GetConvar('inventory:versioncheck', 'true') == 'true' then
	lib.versionCheck('bitpredator/bpt_inventory')
end
...
function server.setPlayerInventory(player, data)
	...
	if inv then
		...
		TriggerClientEvent('bpt_inventory:setPlayerInventory', player.source, Inventory.Drops, inventory, totalWeight, inv.player)
	end
end
exports('setPlayerInventory', server.setPlayerInventory)
AddEventHandler('bpt_inventory:setPlayerInventory', server.setPlayerInventory)
...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	local left = Inventory(source) --[[@as BptInventory]]
...
---@param invType string
---@param data string|number|table
lib.callback.register('bpt_inventory:openInventory', function(source, invType, data)
	return openInventory(source, invType, data)
end)

---@param netId number
lib.callback.register('bpt_inventory:isVehicleATrailer', function(source, netId)
...
exports('forceOpenInventory', function(playerId, invType, data)
	...
	if left and right then
		TriggerClientEvent('bpt_inventory:forceOpenInventory', playerId, left, right)
		return right.id
	end
end)
...
lib.callback.register('bpt_inventory:buyLicense', function(source, id)
...
lib.callback.register('bpt_inventory:getItemCount', function(source, item, metadata, target)
	local inventory = target and Inventory(target) or Inventory(source)
	return (inventory and Inventory.GetItem(inventory, item, metadata, true)) or 0
end)

lib.callback.register('bpt_inventory:getInventory', function(source, id)
...
lib.callback.register('bpt_inventory:useItem', function(source, itemName, slot, metadata)
	local inventory = Inventory(source) --[[@as BptInventory]]
	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			local success = lib.callback.await('bpt_inventory:usingItem', source, data)
...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		local inventory = Inventory(args.target) --[[@as BptInventory]]
...
lib.addCommand('clearevidence', {
	...
	if hasPermission then
		MySQL.query('DELETE FROM bpt_inventory WHERE name = ?', {('evidence-%s'):format(args.evidence)})
	end
end)
...
lib.addCommand('viewinv', {
	...
	if playerInventory and inventory then
		...
		TriggerClientEvent('bpt_inventory:viewInventory', source, inventory)
	end
end)